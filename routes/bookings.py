...
from models import RoomAvailabilityRequest, RoomAvailabilityResponse
...
def prepare_input_data(request_date, room_name):
    ...
    room_code = room_mapping.get(room_name)
    if room_code is None:
        raise ValueError(f"Room name '{room_name}' not recognized.")

    # This should produce a 1D array (or list) for a single sample
    input_data = [day_of_week, month, week_of_year, room_code]
    return input_data  # Not wrapping in another list here



@bookings.post("/predict-availability/", response_model=RoomAvailabilityResponse)
async def predict_availability(request: RoomAvailabilityRequest):
    try:
        input_data = prepare_input_data(request.date.isoformat(), request.room_name)
        prediction = model.predict([input_data])[0]  # Get the single prediction

        # Ensure the prediction is a single value
        assert isinstance(prediction, (int, np.int64)), "Prediction is not a single value"
        ...
        return {
            ...
            "availability": {
                'nineToEleven': availability,
                'elevenToOne': availability,
                'oneToThree': availability,
                'threeToFive': availability
            }
        }
    except AssertionError as ae:
        # Handling the case where the model does not return a single prediction
